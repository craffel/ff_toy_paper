...
First, the state $h_t$ was computed from the input at each time step $x_t$ by $h_t = \max(W_{xh}x_t + b_{xh}, 0.01)$ where $W_{xh} \in \mathbb{R}^{D \times 2}, b_{xh} \in \mathbb{R}^D$.
...
We then computed an intermediate vector $s = \max(W_{cs}c + b_{cs}, 0.01)$ where $W_{cs} \in \mathbb{R}^{D \times D}, b \in \mathbb{R}^D$ from which the output was computed as $y = \max(W_{sy}s + b_{sy}, 0.01)$ where $W_{sy} \in \mathbb{R}^{1 \times D}$, $b_{sy} \in \mathbb{R}$.
